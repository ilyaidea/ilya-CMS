...
class ModuleServices extends Component
{
    ...
    protected function setConfig($path)
    {
        ...
        if(file_exists($path. '/config/config.php'))
        {
            ...
            /** @var Config $coreConfig */
            $coreConfig = $coreConfig->merge($moduleConfig);
        }
    ...
    protected function setAcl()
    {
        ...
        foreach($resources as $role => $resource)
        {
            ...
            {
                $controllerClass = ModuleManager::convertControllerNameToControllerClass($controller, $this->namespace);
                $acl->allow($roles[$role], $controllerClass, $actions);
            }
        }
        return $acl;
    }
