...
//            'script_url' => $this->url->get($this->router->getRewriteUri()),
    ...
    {
        if($this->request->isOptions() || $this->request->isHead())
            $this->head();
        elseif($this->request->isGet())
        {
            $this->get($this->options['print_response']);
        }
        elseif($this->request->isPatch() || $this->request->isPut() || $this->request->isPost())
    ...
    protected function get_format_file($fileName)
    {
        if($fileName)
        {
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
        ...
        return array_values(array_filter(array_map(
            ...
            $this->scanFilesFromGet()
        )));
    }

    protected function scanFilesFromGet()
    {
        $result = [];
    ...
    protected function handle_image_file($file_path, $file) {
        ...
        foreach ($this->options['image_versions'] as $version => $options) {
            if ($this->create_scaled_image($file->blob_name, $version, $options)) {
                if (!empty($version)) {
                    $file->{$version.'Url'} = $this->get_download_url(
                        $file->blob_name,
                        $version
            ...
            $index = null, $content_range = null)
    {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            $file->format = $this->get_format_file($file->name);
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $blob->setSize($file->size);
                    $blob->setFormat($file->format);

                    if($blob->save())
                    {
                        $file_path = $upload_dir. $blob->getId().'.'.$file->format;
                        $file->name = $blob->getId().'.'.$file->format;
                        $file->blob_name = $blob->getId().'.'.$file->format;
                        $file->blob_id = $blob->getId();
            ...
            if ($file_size === $file->size) {
                $file->url = $this->get_download_url($file->blob_name);
                ...
                if (!$content_range && $this->options['discard_aborted_uploads']) {
                    // delete file from blobs table
                    if($file->blob_id)
                    {
                        $img = Blobs::findFirstById($file->blob_id);
                        $img->delete();
                    }
                    unlink($file_path);
    ...
    protected function get_version_param() {
        return $this->basename(stripslashes($this->request->get('version')));
    }
    ...
    protected function get_file_name_param() {
        ...
        return $this->basename(stripslashes($this->request->get($name)));
    }

    protected function get_file_names_params() {
        $params = $this->request->get($this->options['param_name']);
        if (!$params) {
            return null;
        }
    ...
    public function get($print_response = true) {
        if ($print_response && $this->request->get('download')) {
    ...
    public function post($print_response = true) {
        if ($this->request->isGet() && $this->request->get('_method') === 'DELETE') {
    ...
    public function delete($print_response = true) {
        ...
        foreach ($file_names as $file_name) {
            ...
            if ($success) {
                // delete from db blobs table
                if(is_string($file_name))
                {
                    $exp_name = explode('.', $file_name);
                    $blobId = $exp_name[0];
                    $blob = Blobs::findFirstById($blobId);
                    if($blob)
                    {
                        $blob->delete();
                    }
